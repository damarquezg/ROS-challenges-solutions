...
void sendGoal(
	actionlib::SimpleActionClient<puppet_master_challenge::MoveAction> &client,
	const turtlesim::Pose &destination,
	const float &turtle_x,
	const float &turtle_y)
{
	puppet_master_challenge::MoveGoal goal;
	...
	client.sendGoal(goal,
		[goal, &turtle_x, &turtle_y](
			...
			const puppet_master_challenge::MoveResultConstPtr &result)
		{
...
void waitForActionServer(const actionlib::SimpleActionClient<puppet_master_challenge::MoveAction> &client)
{
	ros::Rate rate(10);
...
int main(int argc, char **argv)
{
	...
	actionlib::SimpleActionClient<puppet_master_challenge::MoveAction> action_client("move", false);
